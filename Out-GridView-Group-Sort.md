...
Eventually we will want to enrich our data in which case creating a custom object will be necessary. Below we can use that technique to better group, sort, and visualize. The snippet below will show the users producing the most logon events:
...
Many of the commands above are only the basics. There are several utilities (see SysInternals) that do a far better job at displaying security related information. However we might not always have those tools available and in a pinch we can easily make basic versions; this script below can show us many common auto run locations in Windows commonly used by malware and will enable us to quickly disable entries:
...
There are many more locations that are better covered by SysInternals `autorunc.exe` (command line version of SysInternals `Autoruns.exe`), and when we have access to these tools we can use powershell to easily leverage their output. `Autoruns.exe` has many GUI fanstastic features but if you are forced to work on the command line, we can use the autorunsc.exe to output the information in CSV format, and we can use PowerShell (and it's cmdlet's such as Out-GridView) to filter for information we want:
...
And we can still use `Group-Object` and `Sort-Object` to print most common Persistence locations:
```PowerShell
.\autorunsc64.exe -c -nobanner > autorunOutput.csv
$data = import-csv -Path .\autorunOutput.csv 
$data | Group-Object "Entry Location" -NoElement | Sort-Object Count -Descending | Format-Table -AutoSize
```

One of my favorite ways to investigate each entry on this list we can wrap a while true loop while selecting one or more things from the `Out-GridView` at a time. We can do that with the script below (also in `Demo-CSV.ps1`) to pipe my `Out-GridView` selection to a clipboard, and re-launch/re-populate the list without our selection:
